...
const getRandomColorFromText = (text) => {
  ...
  const startInd = Math.floor(text.length / 1.25);
  const endInd = startInd + 6;

  const hex = text
    .split('')
    .map((x) => (
      Number(parseInt(x.charCodeAt(0), 28)).toString(16)
    ))
    .join('')
...
const serializeSelectedDatetime = (selectedDatetimeObj) => (
  flat(selectedDatetimeObj.map((eachPar) => (
